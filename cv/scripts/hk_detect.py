...
def button_callback(data):
    global button_number,catch_image_flag,image_use,image
...
def get_image(data_buf, nPayloadSize):
    ...
    image = image.reshape((2048, 3072, -1))  # 根据自己分辨率进行转化
    return image,get_flag
...
if __name__ == "__main__":
    ...
    while True:
        ...
        image,get_success = get_image(data_buf, nPayloadSize)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # 默认是BRG，要转化成RGB，颜色才正常


        image_resized = cv2.resize(image_use,(600,400))
